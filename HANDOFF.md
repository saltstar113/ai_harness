...
| 文件 | 作用 |
|------|------|
| `SPEC.md`（1169 行） | 11 章完整设计，含问题陈述、5 个 INVEST 用户故事、功能/非功能规约、架构、数据模型、凭据与分发、领域与机制设计 |
| `PLAN.md`（2520 行） | 18 个 task 的 TDD 实现计划，全部 checkbox 已标记完成，含 commit hash 记录 |
| `SPEC_PROCESS.md`（330+ 行） | 与 Superpowers 协作的 3+3 轮迭代记录、冷启动验证、修订 diff、反思 |
| `AGENT_LOG.md`（310+ 行） | 按时间线记录的完整开发日志 |
